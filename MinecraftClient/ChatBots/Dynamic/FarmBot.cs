	...
	public class FarmBot : ChatBot
	{
		...
		public override void GetText(string text)
		{
			...
			if (!IsPrivateMessage(text, ref message, ref username) && !IsChatMessage(text, ref message, ref username))
			{
				...
				{
					if (GetOnlinePlayers().Contains(Settings.Bots_Owners[0]))
					{
						...
						if (balanceForPay)
						{
							...
							SendText(string.Format("/pay {0} {1}", Settings.Bots_Owners[0], pay));
					...
					{
						SendText(string.Format("/gc {0} is not online.", Settings.Bots_Owners[0]));
					}
				}
